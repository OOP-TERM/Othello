#ifndef _MNT_C_USERS_USER_OODLAB_TERM_PROJECT_OTHELLO_JUDGE_H_
#define _MNT_C_USERS_USER_OODLAB_TERM_PROJECT_OTHELLO_JUDGE_H_
#include <string>
#include "Player.h"
#include "Board.h"

/* 게임 진행 당 필요한 심판은 1명이므로 sigleton pattern 적용 */
class Judge {
 public:
   void StartGame();
   static Judge* GetInstance();
  //  void SetBoard(Board* board);
  //  void SetPlayers(Player* p1, Player* p2);
   void SetScore();
   bool IsGameContinue(char color);
   bool CheckValid(int row, int col, char color);
   bool CheckCnt();
   void SetUsers();
   void CreateBoard();
   bool PlayerInput();
   void SignStone(int row, int col, char color);
   void ModStoneStatus(int row, int col, char color);
   char ChangeTurn(char color);
   std::string CheckWinner();
   std::string GetPlayerName(char color);

 private:
   Judge();
   int turn_, row_, column_;
   std::vector<std::pair<int, int>> candidate_;
   static Judge* instance_;
   Board* board_;
   Player* p1_;
   Player* p2_;
};

#endif // _MNT_C_USERS_USER_OODLAB_TERM_PROJECT_OTHELLO_JUDGE_H_
