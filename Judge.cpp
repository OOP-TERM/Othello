...
void Judge::StartGame() {
  ...
  while (true) {
    ...
    if (this->candidate_.size() == 0) { //후보군이 없으므로 턴이 넘어가야함.
      ...
      std::cout << "Player " << this->GetPlayerName(turn_flag) << "'s Turn" << std::endl;
      continue;
    }
    board_->PrintBoard();
    this->SetScore();
    std::cout << "\n" << p1_->GetName() << "(" << p1_->GetColor() << ") " << p1_->GetScore() << " : ";
    std::cout << p2_->GetScore() << " " << p2_->GetName() << "(" << p2_->GetColor() << ")" << std::endl;
    std::cout << "Player " << this->GetPlayerName(turn_flag) << "'s Turn" << std::endl;
...
void Judge::SetUsers() {
  ...
  std::cout << "Enter Player1's color (B or W) : ";
  std::cin >> temp_color;
  ...
  std::cin >> temp_username;
  std::cout << "Enter Player2's color (B or W) : ";
  std::cin >> temp_color;
